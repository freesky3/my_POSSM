...
class POSSMOutputDecoder(nn.Module):
    ...
    def forward(self, h, freqs_cos, freqs_sin):
        ...
        # result shape: (Batch, max_bin-k+1, input_size, k) -> 调整为 (Batch, max_bin-k+1, k, input_size)
        h_windowed = h.unfold(1, self.k_history, 1).permute(0, 1, 3, 2)
        
        k = self.k_proj(h_windowed)
        v = self.v_proj(h_windowed)
        
        # Reshape for Multi-head: (Batch, max_bin-k+1, k, Heads, Head_Dim)
        k = k.view(batch_size, max_bin-self.k_history+1, self.k_history, self.num_key_value_heads, self.head_dim)
        v = v.view(batch_size, max_bin-self.k_history+1, self.k_history, self.num_key_value_heads, self.head_dim)
        # q_base: (Batch, max_bin, bin_size, embed_dim)
        q_base = self.output_queries.unsqueeze(0).expand(batch_size, max_bin-self.k_history+1, -1, -1)
        q = self.q_proj(q_base)  
        # Reshape: (Batch, max_bin-k+1, bin_size, Heads, Head_Dim)
        q = q.view(batch_size, max_bin-self.k_history+1, self.bin_size, self.num_attention_heads, self.head_dim)
        
        # --- Apply RoPE to Queries --- 
        q = RotaryEmbedding.apply_rotary_pos_emb(q, k, freqs_cos, freqs_sin, is_decoder=True)
        q = q.transpose(2, 3) # (Batch, max_bin-k+1, Heads, bin_size, Head_Dim)
        k = repeat_kv(k, self.n_rep).transpose(2, 3) # (Batch, max_bin-k+1, Heads, k_History, Head_Dim)
        v = repeat_kv(v, self.n_rep).transpose(2, 3) # (Batch, max_bin-k+1, Heads, k_History, Head_Dim)

        # ==========================================
        # 3. Cross Attention
        # ==========================================
        # Scores: (batch_size, max_bin-k+1, Heads, bin_size, k_History)
        scores = (q @ k.transpose(-2, -1)) / math.sqrt(self.head_dim)
        attn_weights = F.softmax(scores, dim=-1)
        attn_weights = self.dropout(attn_weights)
        
        # Output: (batch_size, max_bin-k+1, Heads, bin_size, Head_Dim)
        attn_out = attn_weights @ v 
        
        # Merge Heads
        attn_out = attn_out.transpose(2, 3).reshape(batch_size, max_bin-self.k_history+1, self.bin_size, -1)
        ...
        # 4. Readout (Linear)
        # ==========================================
        # 映射到速度 (batch_size, max_bin-k+1, bin_size, 2)
        vel_pred = self.readout(z_out)
        
        # 展平回时间序列: (batch_size, (max_bin-k+1) * bin_size, 2)
